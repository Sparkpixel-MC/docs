 ...
 * 读取这里配置的 Collections，然后在相关 Collection 相关的 Markdown 文件中，自动生成 permalink
 *
 * collection 的 type 为 `post` 时，表示为文档列表类型（即没有侧边导航栏，有文档列表页）
 * 可用于实现如 博客、专栏等以文章列表聚合形式的文档集合（内容相对碎片化的）
 *
...
const zhGuideDoc = defineCollection({
  ...
  // 如果 前缀不一致，则无法生成侧边栏
  // 所以请确保  markdown 文件中的 permalink 都以 `/` + `linkPrefix` 开头
  linkPrefix: '/guide',
